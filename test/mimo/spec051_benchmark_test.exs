defmodule Mimo.SPEC051.BenchmarkTest do
  ...
  describe "task type detection" do
    ...
    test "detect_task_type handles general queries" do
      ...
      assert is_atom(result)  # Should return some valid task type
    end
  end
  ...
  describe "tiered scoring performance" do
    ...
    test "tier thresholds match spec" do
      ...
      high_importance_item = %{content: "critical data", importance: 1.0, inserted_at: DateTime.utc_now()}
      high_result = HybridScorer.explain_tier(high_importance_item)
      
      # Medium importance item  
      medium_importance_item = %{content: "normal data", importance: 0.5, inserted_at: DateTime.utc_now()}
      medium_result = HybridScorer.explain_tier(medium_importance_item)
      
      # Low importance item
      low_importance_item = %{content: "low priority", importance: 0.1, inserted_at: DateTime.utc_now()}
  ...
  describe "budget allocation efficiency" do
    ...
    test "fit_to_budget reduces items efficiently with integer budget" do
      ...
      items = Enum.map(1..50, fn i ->
        %{
          content: String.duplicate("word ", 100),  # ~500 chars each
          score: 1.0 - i * 0.02,  # Decreasing scores
