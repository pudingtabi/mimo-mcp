defmodule Mimo.NeuroSymbolic.InferenceEngineE2ETest do
  ...
  describe "forward_chain neuro_symbolic + persist" do
    test "uses persisted rule to infer and persist with inferred_by_rule_id" do
      ...
      persisted_for_pred =
        from(r in Rule,
          where: r.validation_status == "validated" and (r.conclusion == ^"reports_to" or like(r.conclusion, ^"%\"predicate\":\"reports_to\"%")),
          select: r
        )
        |> Repo.all()
      ...
      {:ok, symbolic} = InferenceEngine.forward_chain("reports_to", neuro_symbolic: false, persist: false, min_confidence: 0.0)
      assert Enum.any?(symbolic, fn t -> t.subject_id == a and t.object_id == c end)

      {:ok, inferred} = InferenceEngine.forward_chain("reports_to", neuro_symbolic: true, persist: true, min_confidence: 0.0)
      ...
      inferred_a_to_c = Enum.find(inferred, fn t ->
        t.subject_id == a and t.object_id == c and t[:inferred_by_rule_id] != nil
      end)

      assert inferred_a_to_c != nil, "Expected inferred triple from #{a} to #{c} with inferred_by_rule_id"
