defmodule Mimo.Skills.SecureExecutorTest do
  ...
  describe "allowed_command?/1" do
    ...
    test "rejects commands with path" do
      ...
      assert SecureExecutor.allowed_command?("/usr/bin/npx") == true  # basename is npx
      assert SecureExecutor.allowed_command?("../../../bin/sh") == false  # basename is sh
    end
  end
  ...
  describe "execute_skill/1 - environment variables" do
    ...
    test "blocks non-whitelisted env var interpolation" do
      config = %{
        "command" => "npx",
        "args" => ["-y", "@test/package"],
        "env" => %{"MY_VAR" => "${SECRET_KEY}"}  # SECRET_KEY not in allowed list
      }
    ...
    test "validates env var name format" do
      config = %{
        "command" => "npx",
        "args" => ["-y", "@test/package"],
        "env" => %{"invalid-name" => "value"}  # Hyphens not allowed
      }
