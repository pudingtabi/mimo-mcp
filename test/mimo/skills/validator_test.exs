defmodule Mimo.Skills.ValidatorTest do
  ...
  describe "validate_config/1 - env validation" do
    ...
    test "rejects invalid env var names" do
      ...
      assert {:error, {:invalid_env_var_names, ["invalid-name"]}} = Validator.validate_config(config)
    end
    ...
    test "rejects non-allowed interpolation variables" do
      config = %{
        "command" => "npx",
        "args" => [],
        "env" => %{"KEY" => "${SECRET_PASSWORD}"}  # Not in allowed list
      }
      
      assert {:error, {:invalid_interpolation, "KEY", ["SECRET_PASSWORD"], _}} = Validator.validate_config(config)
    end
    ...
    test "accepts allowed interpolation variables" do
      config = %{
        "command" => "npx",
        "args" => [],
        "env" => %{"KEY" => "${EXA_API_KEY}"}  # In allowed list
      }
  ...
  describe "valid_env_var_name?/1" do
    ...
    test "returns false for invalid names" do
      refute Validator.valid_env_var_name?("my-var")  # Hyphen not allowed
      refute Validator.valid_env_var_name?("123_VAR")  # Can't start with number
      refute Validator.valid_env_var_name?("var")  # Must be uppercase
