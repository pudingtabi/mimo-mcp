...
**CRITICAL: When using Mimo via MCP (Model Context Protocol), you access ALL Mimo tools through a single unified interface.**
...
This tool acts as an intelligent gateway to ALL 18 Mimo tools. You describe what you want in natural language, and Mimo routes it to the appropriate internal tool.
...
| Internal Tool Syntax (DON'T USE via MCP) | MCP Interface (USE THIS) |
|------------------------------------------|--------------------------|
| `memory operation=search query="x"` | `mimo-cognitive-agent prompt="Search memory for x"` |
| `file operation=read path="x.ex"` | `mimo-cognitive-agent prompt="Read file x.ex"` |
| `code operation=definition name="foo"` | `mimo-cognitive-agent prompt="Find definition of foo"` |
| `terminal command="mix test"` | `mimo-cognitive-agent prompt="Run command 'mix test'"` |
| `knowledge operation=query` | `mimo-cognitive-agent prompt="Query knowledge graph for..."` |
| `reason operation=guided` | `mimo-cognitive-agent prompt="Start guided reasoning for..."` |

**Remember:** When using Mimo via MCP, there's only ONE tool: `mimo-cognitive-agent`

The internal tool syntax (operation=...) is for direct Elixir API access or stdio mode, NOT for MCP clients like GitHub Copilot CLI.
