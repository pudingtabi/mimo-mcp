defmodule Mix.Tasks.Robustness.Check do
  ...
  defp output_json(results, failures) do
    output = %{
      results: results,
      failures: Enum.map(failures, fn {:error, file, reason} ->
      ...
      summary: %{
        ...
        average_score: if(results != [], do: Enum.sum(Enum.map(results, & &1.score)) / length(results), else: 0)
      }
    }
  ...
  defp format_red_flags_section([]), do: """
  ...
  defp format_red_flags_section(flags) do
    ...
    flag_list = Enum.map(grouped, fn {id, instances} ->
  ...
  defp format_recommendations(results) do
    ...
    if all_recommendations == [] do
      ...
      recs = Enum.map(all_recommendations, fn r ->
        "1. #{r.message}"
      end)
      |> Enum.join("\n")
