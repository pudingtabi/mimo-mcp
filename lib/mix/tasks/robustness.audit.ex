defmodule Mix.Tasks.Robustness.Audit do
  ...
  defp format_markdown(report) when is_map(report) do
    ...
    **Generated:** #{report.generated_at}
    ...
    | Low Risk (≥ 60) | #{summary.low_risk_files} |
  ...
  defp check_threshold(report, threshold) when is_map(report) do
    ...
    if avg_score < threshold do
      Mix.shell().error("\n❌ Audit FAILED: Average score #{avg_score} < threshold #{threshold}")
      exit({:shutdown, 1})
    else
      Mix.shell().info("\n✅ Audit PASSED: Average score #{avg_score} >= threshold #{threshold}")
    end
  end
