defmodule Mimo.Skills.MemoryContext do
  ...
  defp format_age(datetime) do
    ...
    cond do
      ...
      diff_seconds < 2592000 -> "#{div(diff_seconds, 604800)} weeks ago"
      true -> "#{div(diff_seconds, 2592000)} months ago"
    end
  end
  ...
  defp extract_concepts_from_path(path) do
    ...
    # Auth-related
    concepts = if String.contains?(path_lower, ["auth", "login", "session", "jwt", "oauth"]) do
    ...
    # Config-related
    concepts = if String.contains?(path_lower, ["config", "settings", "env", ".env"]) do
    ...
    # Test-related
    concepts = if String.contains?(path_lower, ["test", "spec", "_test", ".test"]) do
    ...
    # API-related
    concepts = if String.contains?(path_lower, ["api", "route", "endpoint", "controller"]) do
    ...
    # Database-related
    concepts = if String.contains?(path_lower, ["model", "schema", "migration", "repo", "database"]) do
  ...
  defp extract_notable_args(parts) do
    parts
    |> Enum.drop(1)  # Skip the command itself
    |> Enum.filter(fn arg ->
  ...
  defp format_file_context(memories, path) do
    ...
    %{
      ...
      count: count,
      summary: summary,
      suggestion: "💡 Consider this context when working with #{filename}. Store new insights with `memory operation=store`."
    }
  end
  ...
  defp format_command_context(memories, command) do
    ...
    %{
      ...
      count: count,
      summary: summary,
      suggestion: "💡 Consider this context before running `#{base_cmd}`. Store important results with `memory operation=store category=action`."
    }
  end

  defp empty_context(type, target) do
    target_name = case type do
    ...
    %{
      ...
      suggestion: "💡 This is new territory. Store valuable insights with `memory operation=store` as you learn."
    }
  end
