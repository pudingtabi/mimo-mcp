defmodule Mimo.MetaCognitiveRouter do
  ...
  def classify(query) when is_binary(query) do
    ...
    total = procedural_score + semantic_score + episodic_score + 0.01  # avoid div/0
    ...
    {primary_store, confidence} = scores
    |> Enum.max_by(fn {_k, v} -> v end)
    
    secondary_stores = scores
  ...
  defp score_procedural(tokens, query) do
    ...
    code_patterns = [
      ...
      ~r/\([^)]*\)/,  # function calls
      ~r/_[a-z]+/     # snake_case identifiers
    ]
  ...
  defp generate_reasoning(store, tokens) do
    matched_keywords = case store do
