defmodule Mimo.Brain.MemoryAuditor do
  ...
  def find_contradictions(limit \\ 20) do
    ...
    negation_query = from e in Engram,
      where: fragment(
        "? LIKE '%not %' OR ? LIKE '%no %' OR ? LIKE '%never%' OR ? LIKE '%don''t%' OR ? LIKE '%doesn''t%' OR ? LIKE '%cannot%' OR ? LIKE '%won''t%'",
        e.content, e.content, e.content, e.content, e.content, e.content, e.content
      ),
      limit: ^limit,
      select: e
  ...
  def find_obsolete_candidates(days_old \\ 90) do
    ...
    query = from e in Engram,
      ...
      where: is_nil(e.superseded_at),  # Not already superseded
    ...
    recs = cond do
      dup_count > 50 ->
        recs ++ [%{
          ...
          recommendation: "Implement automatic deduplication on store. Consider adding content hash check."
        }]
      dup_count > 10 ->
        recs ++ [%{
          severity: :medium,
          issue: "Moderate duplicate count (#{dup_count})",
          recommendation: "Review duplicates and consider manual cleanup or content normalization."
        }]
      true ->
        recs
    end
    ...
    recs = cond do
      contra_count > 10 ->
        recs ++ [%{
          ...
          recommendation: "Review contradictions manually - may indicate evolving understanding or genuine conflicts. Consider TMC supersession."
        }]
      contra_count > 0 ->
        recs ++ [%{
          severity: :low,
    ...
    recs = cond do
      obsolete_count > 30 ->
        recs ++ [%{
          ...
          recommendation: "Consider more aggressive decay parameters or implement memory surgery for cleanup."
        }]
      obsolete_count > 10 ->
        recs ++ [%{
          severity: :low,
          issue: "Some obsolete memory candidates (#{obsolete_count})",
          recommendation: "Memory decay is working normally. Consider reviewing oldest candidates."
        }]
      true ->
        recs
    end
  ...
  defp potentially_contradicts?(content_a, content_b) do
    ...
    negation_pattern = ~r/(^|\s)(not|no|never|don't|doesn't|cannot|won't|isn't|aren't|wasn't|weren't|hasn't|haven't|hadn't|shouldn't|wouldn't|couldn't|can't)(\s|$)/i
