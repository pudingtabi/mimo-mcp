defmodule Mimo.Brain.Cleanup do
  ...
  defp run_cleanup(state) do
    ...
    total_removed = stats.old_memories_removed + 
  ...
  defp cleanup_low_importance_memories(config) do
    ...
    if count > 0 do
      Logger.info("Removed #{count} low-importance memories older than #{config.low_importance_ttl_days} days")
    end
  ...
  defp enforce_memory_limit(config) do
    ...
    if current_count > config.max_memory_count do
      ...
      Logger.warning("Memory limit exceeded (#{current_count}/#{config.max_memory_count}): removed #{count} memories")
      count
    else
      0
    end
  ...
  defp calculate_stats do
    ...
    by_importance = %{
      high: Repo.one(from e in Engram, where: e.importance >= ^@high_importance_threshold, select: count(e.id)) || 0,
      medium: Repo.one(from e in Engram, where: e.importance >= ^@low_importance_threshold and e.importance < ^@high_importance_threshold, select: count(e.id)) || 0,
      low: Repo.one(from e in Engram, where: e.importance < ^@low_importance_threshold, select: count(e.id)) || 0
    }
    
    oldest = Repo.one(from e in Engram, order_by: [asc: e.inserted_at], limit: 1, select: e.inserted_at)
    newest = Repo.one(from e in Engram, order_by: [desc: e.inserted_at], limit: 1, select: e.inserted_at)
    ...
    %{
      ...
      usage_percent: if(get_config(:max_memory_count) > 0, do: total / get_config(:max_memory_count) * 100, else: 0)
    }
  end
  ...
  defp build_config(opts) do
    %{
      ...
      low_importance_ttl_days: Keyword.get(opts, :low_importance_ttl_days, get_config(:low_importance_ttl_days)),
  ...
  defp get_config(key) do
    ...
    case key do
      :default_ttl_days -> Keyword.get(config, :default_ttl_days, @default_ttl_days)
      :low_importance_ttl_days -> Keyword.get(config, :low_importance_ttl_days, @low_importance_ttl_days)
      :max_memory_count -> Keyword.get(config, :max_memory_count, @max_memory_count)
      :cleanup_interval_ms -> Keyword.get(config, :cleanup_interval_ms, @cleanup_interval_ms)
    end
  end
  ...
  defp emit_cleanup_telemetry(stats) do
    :telemetry.execute(
      ...
      %{
        ...
        total_removed: stats.old_memories_removed + stats.low_importance_removed + stats.limit_enforcement_removed,
