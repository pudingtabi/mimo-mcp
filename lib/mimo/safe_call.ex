  ...
  def genserver(name, message, opts \\ []) do
    ...
    result =
      case Process.whereis(name) do
        nil ->
          unless silent,
            do: Logger.debug("[SafeCall] Process #{inspect(name)} not running, using fallback")
        ...
        _pid ->
          try do
            ...
            :exit, {:noproc, _} ->
              unless silent,
                do: Logger.debug("[SafeCall] Process #{inspect(name)} died during call")
            ...
            :exit, reason ->
              unless silent,
                do: Logger.warning("[SafeCall] Exit calling #{inspect(name)}: #{inspect(reason)}")
