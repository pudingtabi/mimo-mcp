defmodule Mimo.Robustness.IncidentParser do
  ...
  def generate_pattern_rule(incident) do
    ...
    if patterns == [] do
      ...
      rules = Enum.map(patterns, fn pattern ->
        %{
  ...
  def parse_dec6_incidents do
    ...
    incidents = [
      %{
        ...
        date: ~D[2025-12-06],
        severity: :high,
        root_cause: "Added synchronous health check in stdio.ex to ensure services ready. GenServer.call during init created circular dependency that blocked startup indefinitely.",
        locations: [
        ...
        fix: "Removed blocking health check from init. Services now use defensive checks at point of use with Process.whereis guards and try/catch around GenServer.call.",
        prevention: "Never use GenServer.call in init/start_link functions. Use defensive checks at runtime with fallbacks."
      },
      %{
        title: "Node Wrapper Compilation Check Fragility",
        date: ~D[2025-12-06],
        severity: :high,
        root_cause: "Used bash/bc/printf to check BEAM file staleness. External command dependencies failed in some environments. No graceful fallback when commands failed.",
        locations: [
          %{file: "bin/mimo-node-wrapper.js", lines: "needsCompilation function", function: "needsCompilation"}
        ],
        patterns: [
          %{name: "exec_sync_bash", category: :external_dep},
          %{name: "no_fallback_on_error", category: :no_fallback}
        ],
        fix: "Rewrote as pure Node.js using fs.statSync for mtime comparison. Added graceful fallback - if check fails, assume up-to-date and start anyway.",
        prevention: "Use pure language features instead of external commands. Always add try/catch with graceful fallback for non-critical operations."
      }
    ]
  ...
  defp extract_locations(content) do
    ...
    if section == "" do
      ...
      file = case Regex.run(~r/File:\s*(.+)$/m, section) do
        [_, f] -> String.trim(f)
        _ -> nil
      end
      
      lines = case Regex.run(~r/Lines?:\s*(.+)$/m, section) do
        [_, l] -> String.trim(l)
        _ -> nil
      end
      
      function = case Regex.run(~r/Function:\s*(.+)$/m, section) do
        [_, f] -> String.trim(f)
        _ -> nil
      end
  ...
  defp extract_patterns(content) do
    ...
    if section == "" do
      ...
      name = case Regex.run(~r/Pattern Name:\s*(.+)$/m, section) do
        [_, n] -> n |> String.trim() |> String.downcase() |> String.replace(" ", "_")
        _ -> "unknown_pattern"
      end
      
      category = case Regex.run(~r/Category:\s*(.+)$/m, section) do
        [_, c] -> 
  ...
  defp teach_to_knowledge_graph(incident) do
    ...
    teachings = teachings ++ Enum.map(incident.patterns, fn pattern ->
      "Pattern #{pattern.name} caused incident #{incident.title}"
    end)
    
    # Teach file -> contains_pattern -> pattern
    teachings = teachings ++ Enum.flat_map(incident.locations, fn loc ->
    ...
    case Code.ensure_loaded(Mimo.Brain.Memory) do
      {:module, _} ->
        results = Enum.map(teachings, fn text ->
          Mimo.Brain.Memory.store(%{
            content: text,
            category: :fact,
            importance: 0.7
