defmodule Mimo.Robustness.KnowledgeIntegration do
    ...
    with {:ok, incidents} <- IncidentParser.parse_dec6_incidents() do
      results =
        Enum.map(incidents, fn incident ->
          ...
          location_results =
            Enum.map(incident.locations, fn loc ->
              if loc.file do
  ...
  def link_incident(incident) do
    ...
    teach(
      "Incident #{incident.title} occurred on #{incident.date} with severity #{incident.severity}"
    )
