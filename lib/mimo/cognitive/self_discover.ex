defmodule Mimo.Cognitive.SelfDiscover do
  ...
  defp default_modules do
    [
      Enum.at(@atomic_modules, 0),   # Break down into parts
      Enum.at(@atomic_modules, 9),   # Generate sub-tasks
      Enum.at(@atomic_modules, 19),  # Make implicit explicit
      Enum.at(@atomic_modules, 30),  # Think step by step
      Enum.at(@atomic_modules, 33)   # Understand what is asked
    ]
  end
  ...
  defp adapt_modules(task, selected_modules) do
    ...
    case LLM.complete(prompt, max_tokens: 500, raw: true) do
      ...
      {:error, reason} ->
        ...
        {:ok, modules_text}  # Fallback to unadapted modules
    end
  end
  ...
  defp default_structure do
    %{
      "step_1_understand" => "Carefully read and understand what the task is asking. Identify any implicit requirements.",
      "step_2_identify_implicit" => "Look for hidden requirements or assumptions. What is NOT explicitly stated but required?",
  ...
  defp task_signature(task) do
    ...
    normalized = task
    |> String.downcase()
    |> String.replace(~r/\d+/, "N")  # Replace numbers with N
