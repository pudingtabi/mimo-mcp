defmodule Mimo.Cognitive.RephraseRespond do
  ...
  def needs_rephrasing?(question) when is_binary(question) do
    ...
    patterns = [
      ...
      # Ambiguity patterns
      ~r/\bthis\b.*\bthat\b/i,  # Vague references
      ~r/how many.*\?.*\?/i,    # Multiple questions
  ...
  defp build_respond_prompt(original, rephrase_result, include_reasoning) do
    meta_task_note = if rephrase_result.is_meta_task do
      """
    ...
    implicit_note = if length(rephrase_result.implicit_requirements) > 0 do
    ...
    reasoning_instruction = if include_reasoning do
