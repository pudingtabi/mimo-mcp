defmodule Mimo.Cognitive.MetaTaskHandler do
  ...
  defp handle_meta_task(task, guidance, strategy, opts) do
    case strategy do
      :auto ->
        try_techniques_in_order(task, guidance, opts)

      :combined ->
        apply_combined_strategy(task, guidance, opts)
  ...
  defp apply_combined_strategy(task, guidance, opts) do
    ...
    rephrased_task =
      case RephraseRespond.rephrase(task) do
        {:ok, %{rephrased: r}} -> r
        _ -> task
      end
    ...
    case SelfDiscover.discover(enhanced_task) do
      {:ok, discovery} ->
        case SelfDiscover.solve(enhanced_task, discovery.reasoning_structure,
  ...
  defp fallback_to_reasoner(task, opts \\ []) do
    ...
    reasoner_opts =
      if Keyword.get(opts, :is_from_handler, false) do
